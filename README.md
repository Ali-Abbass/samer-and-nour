...
| **Opening video** | `ASSETS.introVideo` → replace `public/videos/intro.mp4`. It loads paused behind a "tap to open" control, showing `public/images/intro-poster.jpg` (a still of its first frame — replace it too when you swap the video) until the file has downloaded; the tap plays it with sound and primes the music. When it ends, it fades into the invitation. |
