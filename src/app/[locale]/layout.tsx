...
export const viewport: Viewport = {
  ...
  initialScale: 1,
  themeColor: '#ebe8e1',
  /* Paint the whole screen, not the box Safari leaves between its bars.
     Without this iOS insets the page to the safe area — measured on an
     iPhone 15 Pro Max: screen 932px, page 683px, so 249px of the display
     was never ours to draw on and the photo stopped short of the bottom
     bar. With it the backdrop and the scrim run edge to edge and Safari's
     translucent toolbar sits over them, the way apple.com reads.

     The trade is that content can now fall behind the chrome, so
     everything anchored to an edge pads itself with the matching
     `env(safe-area-inset-*)`. Those env() values are zero until this is
     set — `.scene` has been carrying one that never did anything. */
  viewportFit: 'cover',
};
